    ...
    >
      {MENU_ITEMS.map(({ icon: Icon, label, path }) => (
        <button
          ...
          role="menuitem"
        >
          <Icon size={16} className="text-xf-subtle group-hover:text-xf-gold transition-colors" />
          {label}
        </button>
      ))}
      <div className="border-t border-white/10 mt-1 pt-1 pb-1">
        ...
        >
          <LogOut size={16} className="text-xf-subtle group-hover:text-xf-gold transition-colors" />
          Sign Out
        </button>
      </div>
    </motion.div>
